...
exports.handler = async (event, context) => {
    try {
        ...
        const response = await db1.allDocs({
            include_docs: true,
            startkey: 'SVYMFM',
            endkey: 'SVYMFM\ufff0' // This is a special character to create an inclusive range
        });
        
        // Extract the full document objects from the response
        const fieldmobilisers = response.rows.map(row => row.doc);
